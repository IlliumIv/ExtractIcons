...
namespace ExtractIcons
{
	public class DDSImage
	{
		...
		public void Save(string file, Rectangle? rectangle = null)
		{
			if (_image.Format == Pfim.ImageFormat.Rgba32)
				Save<Bgra32>(file, rectangle);
			else if (_image.Format == Pfim.ImageFormat.Rgb24)
				Save<Bgr24>(file, rectangle);
			else
		...
		private void Save<T>(string file, Rectangle? rectangle = null)
			where T : struct, IPixel<T>
		{
			Image<T> image = Image.LoadPixelData<T>(
				_image.Data, _image.Width, _image.Height);
			if (rectangle != null) image.Mutate(i => i.Crop((Rectangle)rectangle));
			image.Save(file);
		}
